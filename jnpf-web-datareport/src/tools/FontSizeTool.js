...
export default class FontSizeTool extends Tool{
    ...
    buildButton(){
        ...
        this.nameButton=$(`<button type="button" class="btn btn-default"
            style="border:none;border-radius:0;padding: 6px 1px 6px 5px;font-size: 12pt;" title="${window.i18n.tools.fontSize.size}">
            <i class="report-icon report-icon-font-size"></i></button>`);
        group.append(this.nameButton);
        const mainBtn=$(`<button type="button" class="btn btn-default dropdown-toggle" style="border:none;border-radius:0;padding: 6px 5px;" data-toggle="dropdown" title="${window.i18n.tools.fontSize.size}">
        ...
        for(let i=1;i<=100;i++){
            let size=$(`<li data="${i}"><a href='javascript:;'>${i}</a></li>`);
            ul.append(size);
